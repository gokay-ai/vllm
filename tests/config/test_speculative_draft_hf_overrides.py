...
@pytest.mark.cpu_test
def test_dict_yarn_override_is_picklable():
    """Mapping-derived draft overrides are pickled with the draft ModelConfig
    into engine-core processes."""
    overrides = {"text_config": {"rope_parameters": _YARN_ROPE_PARAMETERS}}
    composed = SpeculativeConfig.compose_draft_hf_overrides(overrides)
    restored = pickle.loads(pickle.dumps(composed))

    text_config = _make_hf_config(rope_parameters={"rope_type": "default"})
    out = restored(_make_hf_config(text_config=text_config, model_type="llama"))
    assert out.text_config.rope_parameters == _YARN_ROPE_PARAMETERS
